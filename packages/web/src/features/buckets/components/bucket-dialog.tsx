...
import { useCreateBucket, useUpdateBucket } from "../hooks/use-buckets";
...
export function BucketDialog({ open, onClose, editCredential }: BucketDialogProps) {
  ...
  const { data: fullData } = useQuery({
    queryKey: ["buckets", editCredential?.id],
    queryFn: () => bucketsApi.get(editCredential!.id),
  ...
  useEffect(() => {
    if (open && editCredential && fullData?.data) {
      const cred = fullData.data as BucketCredential;
      if (cred.type === "gcs") {
        reset({
          type: "gcs",
          name: cred.name,
  ...
  const inputMode = credType === "gcs" ? (watch as any)("inputMode") as string : undefined;
  ...
  const onSubmit = handleSubmit((data) => {
    if (data.type === "gcs") {
      const payload: BucketCredentialInput = {
        type: "gcs",
        name: data.name,
        provider: "gcs",
      ...
      if (isEdit) {
        updateMutation.mutate(
          { id: editCredential.id, data: payload as Partial<BucketCredential> },
          { onSuccess: () => onClose() }
        );
      } else {
        ...
        {credType === "s3" && (
          <>
            ...
            <Field data-invalid={!!(errors as any).secretAccessKey}>
              <FieldLabel htmlFor="bucket-secret-key">Secret Access Key</FieldLabel>
              <Input
          ...
          <>
            ...
            {inputMode === "file" ? (
              <Field data-invalid={!!(errors as any).serviceAccountJson}>
                <FieldLabel htmlFor="bucket-sa-file">Service Account JSON File</FieldLabel>
                <Input
                  id="bucket-sa-file"
                  ref={fileInputRef}
                  type="file"
              ...
              <Field data-invalid={!!(errors as any).serviceAccountJson}>
                <FieldLabel htmlFor="bucket-sa-json">Service Account JSON</FieldLabel>
                <Textarea
                  id="bucket-sa-json"
